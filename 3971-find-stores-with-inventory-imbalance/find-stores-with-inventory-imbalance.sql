...
WITH filter1 AS (
    ...
    SELECT * FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY store_id ORDER BY price) AS num FROM inventory WHERE store_id IN (SELECT store_id FROM filter1)) x WHERE num = 1 OR num = (SELECT cnt FROM filter1 WHERE x.store_id = filter1.store_id)
), T AS (
    SELECT T1.store_id AS store_id, T2.product_name AS most_exp_product, T1.product_name AS cheapest_product, ROUND(T1.quantity / T2.quantity, 2) AS imbalance_ratio FROM (SELECT store_id, product_name, quantity, price FROM filter2 WHERE num = 1) T1 LEFT JOIN (SELECT store_id, product_name, quantity, price FROM filter2 WHERE num <> 1) T2 ON T1.store_id = T2.store_id WHERE T1.quantity > T2.quantity
)
